...
interface AIPrediction {
  ...
  score_5: number; score_10: number; score_20: number;
...
function buildCombinedPicks(screener: StockData[], predictions: AIPrediction[]): CombinedPick[] {
  ...
  return screener
    .map(stock => {
      ...
      // AI score_10 → 0–50 (clamp tại ±4%)
      const rawAI = Math.max(-4, Math.min(4, ai.score_10));
      const aiScore = Math.round(((rawAI + 4) / 8) * 50);
...
function TopPickCard({ pick, rank }: { pick: CombinedPick; rank: number }) {
  ...
  const bothAgree = techScore >= 30 && ai.signal === "Mua";

  return (
    <div className={clsx(
      "rounded-xl border p-4 flex flex-col gap-3 transition-all",
      rank === 1
        ? "border-amber-600/60 bg-gradient-to-b from-amber-950/30 to-gray-900/50 shadow-lg shadow-amber-900/10"
      ...
      <div className="flex items-start justify-between">
        ...
        <div className={clsx(
          ...
          combinedScore >= 70 ? "border-amber-500 bg-amber-950/30" :
          combinedScore >= 50 ? "border-emerald-600 bg-emerald-950/30" :
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={clsx("text-[10px] font-mono w-10 text-right", ai.score_10 > 0 ? "text-purple-400" : "text-red-400")}>
            {ai.score_10 > 0 ? "+" : ""}{ai.score_10.toFixed(2)}%
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center">
        ...
        <div className="bg-emerald-950/30 border border-emerald-900/30 rounded-lg py-1.5">
          <p className="text-[9px] text-gray-600 uppercase">Mục tiêu</p>
          <p className="text-xs font-bold text-emerald-300">{target}k</p>
        </div>
        <div className="bg-red-950/20 border border-red-900/20 rounded-lg py-1.5">
      ...
      <div className={clsx("rounded-lg p-2.5 mt-1 border", 
        isBuyNow ? "bg-emerald-900/20 border-emerald-700/40" : "bg-amber-900/10 border-amber-700/30"
      )}>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-medium text-gray-400 bg-gray-950/50 px-1.5 py-0.5 rounded border border-gray-800">
            Khuyên giữ: {horizon}
          </span>
        </div>
      ...
      <p className="text-[10px] text-gray-600 text-center mt-1">
        R:R = 1:{rr} · AI 10p: <span className={ai.score_10 > 0 ? "text-purple-400" : "text-red-400"}>{ai.score_10 > 0 ? "+" : ""}{ai.score_10.toFixed(2)}%</span>
      </p>
    </div>
  );
}
    ...
    <main className="min-h-screen bg-black text-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex gap-2 mb-6">
          <button onClick={() => setActiveTab("combined")}
            className={clsx("px-4 py-1.5 rounded-lg text-sm font-medium transition-colors",
              activeTab === "combined" ? "bg-amber-600 text-white" : "bg-gray-800 text-gray-400 hover:text-white")}>
            🏆 Top Picks (Kỹ thuật + AI)
          </button>
          <button onClick={() => setActiveTab("ai")}
            className={clsx("px-4 py-1.5 rounded-lg text-sm font-medium transition-colors",
              activeTab === "ai" ? "bg-purple-700 text-white" : "bg-gray-800 text-gray-400 hover:text-white")}>
            🤖 Bảng Xếp hạng AI
          </button>
        </div>
        ...
        {loading ? (
          ...
          <>
            {!hasAI ? (
              <div className="text-center py-20 rounded-xl border border-gray-800 bg-gray-900/20">
                ...
                <table className="min-w-full text-sm text-left">
                  <thead className="text-xs text-gray-500 uppercase bg-gray-950 border-b border-gray-800">
                    <tr>
                      ...
                      <th className="px-4 py-3">AI 10 phiên</th>
                      <th className="px-4 py-3">AI 20 phiên</th>
                  ...
                  <tbody>
                    {predictions.map((p, idx) => {
                      ...
                      const sigCfg = { Mua: "text-green-300 bg-green-900/40 border-green-700/50", "Theo dõi": "text-amber-300 bg-amber-900/40 border-amber-700/50", Tránh: "text-red-300 bg-red-900/40 border-red-700/50" }[p.signal] ?? "text-gray-400";
                      return (
                        <tr key={p.ticker} className={clsx("border-b border-gray-800/50 hover:bg-gray-800/20", idx % 2 === 0 ? "bg-black/10" : "")}>
                          ...
                          <td className="px-4 py-3"><ScoreBar value={p.score_10} /></td>
                          <td className="px-4 py-3"><ScoreBar value={p.score_20} /></td>
